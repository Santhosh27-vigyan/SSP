...
public void LoginWithInValidCredentails()
{
	...
	LPO.ClickOnLogin().click();
	WebDriverWait wait=new WebDriverWait(driver, Duration.ofSeconds(10));
	wait.until(ExpectedConditions.attributeContains(LPO.InvalidCredentialsErrorMessage(), "style", "display: block;"));
	//assertEquals(LPO.InvalidCredentialsErrorMessage().getAttribute("style"), "display: block;");
...
public void LoginWithInvalidCredentialsUsingRightCaptcha()
{
	...
	assertEquals(LPO.InvalidCredentialsErrorMessage().getText().trim(), "Invalid Username or Password!");
	assertTrue(LPO.GetCaptchaText().isDisplayed());
	wait.until(ExpectedConditions.not(ExpectedConditions.textToBePresentInElement(LPO.GetCaptchaText(), FirstCaptcha)));
